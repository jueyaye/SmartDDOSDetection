...
from onos import ONOS, ONOSOVSSwitch
from pox import POX
...
from simulator import runHosts, runTCPDump, signal_handler
...
class PrefsDialog(tkSimpleDialog.Dialog):
    ...
    def body(self, master):
        ...
        # --UPDATED-- to include the ONOSOVSSwitch
        ...
        self.switchTypeMenu = OptionMenu(self.leftfieldFrame, self.switchType, "Open vSwitch Kernel Mode", 
            "Indigo Virtual Switch", "Userspace Switch", "Userspace Switch inNamespace", "ONOS OVS Switch")
        ...
        elif switchTypePref == 'ONOSOVSSwitch':
            self.switchType.set("ONOSOVSSwitch")
        else:
    ...
    def apply(self):
        ...
        elif sw == 'ONOSOVSSwitch':
            self.result['switchType'] = 'ONOSOVSSwitch'
        else:
...
class SwitchDialog(CustomDialog):
    ...
    def apply(self):
        ...
        elif sw == 'ONOS OVS Switch':
            results['switchType'] = 'ONOSOVSSwitch'
        else:
...
class ControllerDialog(tkSimpleDialog.Dialog):
    ...
    def body(self, master):
        ...
        # Field for Controller type --UPDATED to include our custom controllers--  custom ONOS, custom POX
        Label(master, text="Controller Type:").grid(row=rowCount, sticky=E)
        controllerType = self.ctrlrValues['controllerType']
        
        self.o1 = OptionMenu(master, self.var, "Remote Controller", "In-Band Controller", "OpenFlow Reference", "OVS Controller", "ONOS", "POX")
        ...
        elif controllerType == 'ONOS':
            self.var.set("ONOS")
        elif controllerType == 'POX':
            self.var.set("POX")
        else:
    ...
    def apply(self):
        ...
        if controllerType == 'Remote Controller':
            ...
            self.result['controllerType'] = 'ONOS'
        elif controllerType == 'POX':
            self.result['controllerType'] = 'POX'
        else:
...
class MiniEdit( Frame ):
    ...
    def exportScript( self ):
        ...
        # -- update -- the file exporter to include the new controller/switch elements
        ...
        if len(fileName ) > 0:
            ...
            f.write("from onos import ONOS, ONOSOVSSwitch\n")
            f.write("from pox import POX\n")
            ...
            for widget in self.widgetToItem:
                ...
                if 'Controller' in tags:
                    ...
                    if controllerType == 'remote':
                        ...
                        f.write("                      controller=ONOS,\n")
                    elif controllerType == 'POX':
                        f.write("                      controller=POX,\n")
                    else:
            ...
            for widget in self.widgetToItem:
                ...
                if 'Switch' in tags:
                    ...
                    if opts['switchType'] == 'default':
                        ...
                        f.write(", cls=UserSwitch, inNamespace=True")
                    elif opts['switchType'] == 'ONOSOVSSwitch':
                        f.write(", cls=ONOSOVSSwitch")
                    else:
                        f.write(", cls=OVSKernelSwitch")
    ...
    def postStartSetup( self ):
        ...
        # @Reuban @Billy @Alex your entry point is here
        ...
        runTCPDump(self.net, hostFilenames)
        runtime = 0
        runHosts(self.net, clientFilenames, runtime)
        ...
        print hostFilenames
        print clientFilenames
        generate(hostFilenames, clientFilenames, "./logs/KDDMimic.csv", "Kyoto")
        ...
        if self.net is None:
            ...
            for widget in self.widgetToItem:
                ...
                if 'Switch' in tags:
                    ...
                    print type(switchControllers[0]).__name__
