...
def generateKyotoString(connection):
	...
	vector = ',' + str(src_IP) + ',' + str(dst_IP) + ',' + str(src_port) + ',' + str(dst_port) + ',' + str(startTime)
	return vector
	...
	for connection in connections:
		...
		for subCon in connectionSet:
			if set(connection) == set(subCon):
				add = False
				print "WE CHUCKING DID IT LADS, WE'RE INVINCIBLE"
				break
...
def removeDuplicateStrings(strings, setType):
	cleanList = []
	...
	for string in strings:
		if setType == "Kyoto":
			matchStrs.append(string[string.find(',') + 1 : string.rfind(',')])
		else:
			matchStrs.append(string[string.find(',') + 1 : ])

	for i in range(0, len(matchStrs)):
		add = True
	
		matchStr = matchStrs[i]		
	
		for matchSub in matchStrs:

			if matchStr == matchSub:
				print "Nailed it friends"
				add = False
				break

		cleanList.append(strings[i])
...
def generateKDDFromFiles(filenames, clientFilenames, newFileName, setType):
	...
	strings = removeDuplicateStrings(strings, setType)
